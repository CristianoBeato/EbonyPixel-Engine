...
btRenderModelLWO::btRenderModelLWO(void) : idRenderModelStatic()
{
}

btRenderModelLWO::~btRenderModelLWO(void)
{
}

bool btRenderModelLWO::Load(void)
{
	unsigned int failID;
	int failPos;
	lwObject* lwo;

	lwo = lwGetObject(m_name, &failID, &failPos);
	if (lwo == NULL)
		return false;
...
bool btRenderModelLWO::ConvertLWOToModelSurfaces(const st_lwObject * lwo)
{
	...
	{
		common->Warning("ConvertLWOToModelSurfaces: model \'%s\' has bad or missing vertex data", m_name.c_str());
		return false;
	}
	...
	{
		common->Warning("ConvertLWOToModelSurfaces: model \'%s\' has bad or missing uv data", m_name.c_str());
		numTVertexes = 1;
	...
	for (lwoSurf = lwo->surf, i = 0; lwoSurf; lwoSurf = lwoSurf->next, i++)
	{
		...
		for (j = 0; j < layer->polygon.count; j++)
		{
			...
			if (poly->nverts != 3)
			{
				common->Warning("ConvertLWOToModelSurfaces: model %s has too many verts for a poly! Make sure you triplet it down", m_name.c_str());
				continue;
			}
		...
		if (tri->numIndexes > layer->polygon.count * 3)
			common->FatalError("ConvertLWOToModelSurfaces: index miscount in LWO file %s", m_name.c_str());

		if (tri->numVerts > layer->polygon.count * 3)
			common->FatalError("ConvertLWOToModelSurfaces: vertex miscount in LWO file %s", m_name.c_str());
		...
		modelSurf = &this->m_surfaces[mergeTo[i]];
