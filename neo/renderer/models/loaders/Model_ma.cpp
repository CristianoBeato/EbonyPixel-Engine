...
=================
*/
maModel_t* btRenderModelMA::MA_Load( const char* fileName )
{
	char* buf;
	ID_TIME_T timeStamp;
	maModel_t* ma;
...
/*
=================
btRenderModelMA::MA_Free
=================
*/
void btRenderModelMA::MA_Free( maModel_t* ma )
{
	int					i;
	maObject_t*			obj;
	maMesh_t*			mesh;
...
btRenderModelMA::btRenderModelMA(void) : idRenderModelStatic()
{
}

btRenderModelMA::~btRenderModelMA(void)
{
}

bool btRenderModelMA::Load(void)
{
	maModel_t* ma;
...
=================
*/
bool btRenderModelMA::ConvertMAToModelSurfaces(const struct maModel_s* ma)
{
	maObject_t* 	object;
	maMesh_t* 		mesh;
	maMaterial_t* 	material;
	...
	for (objectNum = 0; objectNum < ma->objects.Num(); objectNum++)
	{
		...
		for (j = 0; j < mesh->numFaces; j++)
		{
			for (k = 0; k < 3; k++)
			{
				...
				if (v < 0 || v >= mesh->numVertexes)
					common->Error("ConvertMAToModelSurfaces: bad vertex index in MA file %s", m_name.c_str());
				...
				if (mesh->numTVertexes != 0)
				{
					...
					{
						common->Error("ConvertMAToModelSurfaces: bad tex coord index in MA file %s", m_name.c_str());
					}
		...
		if (tri->numIndexes > mesh->numFaces * 3)
			common->FatalError("ConvertMAToModelSurfaces: index miscount in MA file %s", m_name.c_str());

		if (tri->numVerts > mesh->numFaces * 3)
			common->FatalError("ConvertMAToModelSurfaces: vertex miscount in MA file %s", m_name.c_str());
