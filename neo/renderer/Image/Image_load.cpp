...
// Beato Begin: generate image from the scrap
void idImage::generateFromBitmap(btBitmapBuffer * buff)
{
	this->GenerateImage(buff->getBuff(), buff->getWidth(), buff->getHeight(), TF_DEFAULT, TR_REPEAT, buff->getUsage());
}
// Beato End
...
//Beato End
	...
	else
	{
		...
		else if( cubeFiles != CF_2D )
		{
			opts.textureType = TT_CUBIC;
			repeat = TR_CLAMP;
			R_LoadCubeImages( GetName(), cubeFiles, NULL, NULL, NULL, &sourceFileTime);
		}
		else
		{
	...
	else
	{
		if( cubeFiles != CF_2D )
		{
			...
			if( !R_LoadCubeImages( GetName(), cubeFiles, pics, &size, NULL, &sourceFileTime ) || size == 0 )
			{
...
void idImage::Reload( bool force )
{
	...
	if( !force )
	{
		ID_TIME_T current;
		if( cubeFiles != CF_2D )
			R_LoadCubeImages( imgName, cubeFiles, NULL, NULL, NULL, &current );
		else
			R_LoadImageProgram( imgName, NULL, NULL, NULL, &current ); // get the current values
...
/*
==============================
btBitmapBuffer::btBitmapBuffer
==============================
*/
btBitmapBuffer::btBitmapBuffer(void)
{
...
/*
==============================
btBitmapBuffer::clear
==============================
*/
void btBitmapBuffer::clear(void)
{
	if (m_bitmapPixels != NULL)
	{
...
/*
==============================
btBitmapBuffer::createPixels
==============================
*/
void btBitmapBuffer::createPixels(uint imgWidth, uint imgHeight)
{
	//Valid dimensions
	if (imgWidth > 0 && imgHeight > 0)
	{
		//Get rid of any current Bitmap data
		clear();

		//Create pixels
		uint size = imgWidth * imgHeight;
		m_bitmapPixels = new byte[size];

		//set a "black" pixels
		memset(m_bitmapPixels, 0x00, size);

		//Copy pixel data
		m_bitmapWidth = imgWidth;
		m_bitmapHeight = imgHeight;
	}
}

/*
==============================
btGlyphBitmap::copyPixels
==============================
*/
void btBitmapBuffer::copyPixels(byte * pixels, GLuint imgWidth, GLuint imgHeight)
{
	...
	uint size = imgWidth * imgHeight;
	m_bitmapPixels = new byte[size];
	memcpy(m_bitmapPixels, pixels, size);
...
/*
==============================
btGlyphBitmap::blitPixels
==============================
*/
void btBitmapBuffer::blitPixels(GLuint x, GLuint y, btBitmapBuffer* &destination)
{
	//There are pixels to blit
	if (m_bitmapPixels != NULL && destination->m_bitmapPixels != NULL)
	{
		//Copy pixels rows
		byte* dPixels = destination->m_bitmapPixels;
		byte* sPixels = m_bitmapPixels;
		for (int i = 0; i < m_bitmapHeight; ++i)
		{
			memcpy(&dPixels[(i + y) * destination->m_bitmapWidth + x], &sPixels[i * m_bitmapWidth], m_bitmapWidth);
		}
	}
}
...
/*
==============================
btGlyphBitmap::getBuff
==============================
*/
byte * btBitmapBuffer::getBuff(void) const
{
	return m_bitmapPixels;
}

/*
==============================
btGlyphBitmap::getWidth
==============================
*/
uint btBitmapBuffer::getWidth(void) const
{
...
/*
==============================
btGlyphBitmap::getHeight
==============================
*/
uint btBitmapBuffer::getHeight(void) const
{
...
/*
==============================
btGlyphBitmap::getUsage
==============================
*/
textureUsage_t btBitmapBuffer::getUsage(void) const
{
	return m_bitmapUsage;
}
//Beato End