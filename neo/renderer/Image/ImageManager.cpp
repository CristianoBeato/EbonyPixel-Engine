...
// Beato Begin
idImage * idImageManager::ImageFromBitmapBuffer(const char * _name, btBitmapBuffer * buff)
{
	idImage*	 image = NULL;
	// strip any .tga file extensions from anywhere in the _name
	idStr name = _name;
	...
	for (int i = imageHash.First(hash); i != -1; i = imageHash.Next(i))
	{
		...
		if (name.Icmp(image->GetName()) == 0)
		{
			if (image->bmpBuffer != buff)
				common->DPrintf("WARNING: reused image %s with mixed buffers\n", name.c_str());
			return image;
		}
	}
...
void R_CombineCubeImages_f( const idCmdArgs& args )
{
	...
	for( int frameNum = 1 ; frameNum < 10000 ; frameNum++ )
	{
		...
		for( side = 0 ; side < 6 ; side++ )
		{
			...
			R_LoadImage( filename, &pics[side], &width, &height, NULL, NULL, true );
		...
		//R_WriteImage( filename, combined, width, height * 6);
		R_WriteImage(filename, combined, width, height * 6, TGA);
	}
