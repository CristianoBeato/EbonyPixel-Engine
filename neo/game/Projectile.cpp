...
void idProjectile::Explode( const trace_t& collision, idEntity* ignore )
{
	...
	if( fxdebris )
	{
		const idDict* debrisDict = gameLocal.FindEntityDefDict( "projectile_debris", false );
		if(debrisDict)
		{
			...
			for( int i = 0; i < amount; i++ )
			{
				...
				gameLocal.SpawnEntityDef( *debrisDict, &ent, false );
		...
		debrisDict = gameLocal.FindEntityDefDict( "projectile_shrapnel", false );
		if(debrisDict)
		{
			int amount = gameLocal.random.RandomInt( fxdebris );
			for( int i = 0; i < amount; i++ )
...
void idGuidedProjectile::Launch( const idVec3& start, const idVec3& dir, const idVec3& pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower )
{
	...
	if( owner.GetEntity() )
	{
		...
		else if( owner.GetEntity()->IsType( idPlayer::Type ) )
		{
			...
			gameLocal.clip.TracePoint( trc, start, end, MASK_SHOT_RENDERMODEL | CONTENTS_BODY, owner.GetEntity() );
			if( trc.fraction < 1.0f )
			{
				enemy = gameLocal.GetTraceEntity( trc );
			}
