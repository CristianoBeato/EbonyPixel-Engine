...
void idAI::Event_LaunchMissile( const idVec3& org, const idAngles& ang )
{
	...
	gameLocal.clip.Translation( trc, start, org, projClip, projClip->GetAxis(), MASK_SHOT_RENDERMODEL, this );
	
	// launch the projectile
	idThread::ReturnEntity( projectile.GetEntity() );
	projectile.GetEntity()->Launch( trc.endpos, axis[ 0 ], vec3_origin );
	projectile = NULL;
	
	TriggerWeaponEffects( trc.endpos );
