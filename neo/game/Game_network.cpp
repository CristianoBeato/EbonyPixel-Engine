...
void idGameLocal::ServerProcessReliableMessage( int clientNum, int type, const idBitMsg& msg )
{
	...
	switch( type )
	{
		...
		case GAME_RELIABLE_MESSAGE_CLIENT_HITSCAN_HIT:
		{
			...
			trace_t trc;
			gameLocal.clip.Translation( trc, muzzleOrigin, targetLocation, NULL, mat3_identity, MASK_SHOT_RENDERMODEL, &attacker );
			
			idEntity* hitEnt = gameLocal.entities[ trc.c.entityNum ];
			if( hitEnt != &victim )
			{
				break;
			}
