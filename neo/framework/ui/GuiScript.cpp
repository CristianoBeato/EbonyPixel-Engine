...
void Script_Transition( idWindow* window, idList<idGSWinVar, TAG_OLD_UI>* src )
{
	...
	if( src->Num() >= 4 )
	{
		...
		{
			vec4->SetEval( false );
			window->AddTransition( vec4, *from, *to, time, pac, pdc);
			//
			//  added float variable
		}
		else if( val )
		{
			val->SetEval( false );
			window->AddTransition( val, *from, *to, time, pac, pdc);
			//
		}
		else
		{
			rect->SetEval( false );
			window->AddTransition( rect, *from, *to, time, pac, pdc);
		}
...
void idGuiScript::FixupParms( idWindow* win )
{
	...
	if( handler == &Script_Set )
	{
		...
		str = dynamic_cast<idWinStr*>( parms[0].var );
		assert( str );
