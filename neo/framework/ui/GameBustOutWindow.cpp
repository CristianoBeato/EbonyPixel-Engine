...
======================
*/
void BOBrick::ReadFromSaveGame( idFile* savefile, idGameBustOutWindow* _game )
{
...
void idGameBustOutWindow::LoadBoardFiles()
{
	...
	{
		byte* pic;
		imageDataType_t type;
		idStr	name = "guis/assets/bustout/level";
		name += ( i + 1 );
		name += ".tga";
		
		R_LoadImage( name, &pic, &w, &h, &type, &time, false );
