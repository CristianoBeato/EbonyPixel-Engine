	...
	for( int i = 0; i < dictionary.Num(); i++ )
	{
		...
		switch( dictionary[i].type )
		{
			case SWF_DICT_IMAGE:
			{
				...
				R_WriteImage( imageExportFileName.c_str(), pngData.Ptr(), width, height, PNG, true, "fs_basepath" );
...
bool idSWF::LoadJSON( const char* filename )
{
	...
	for( SizeType i = 0; i < a.Size(); i++ )
	{
		...
		if( type == "IMAGE" )
		{
			...
			int width, height;
			ID_TIME_T timestamp;
			R_LoadImage( imageName.c_str(), &imageData, &width, &height, NULL, &timestamp, false );
			if( imageData != NULL )
			{
				LoadImage( i, (byte*)imageData, width, height );
