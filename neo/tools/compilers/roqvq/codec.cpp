...
codec::codec() {
	...
	codebooksize = 256;
	codebook2 = (VQDATA **) Mem_ClearedAlloc(256*sizeof(VQDATA *), TAG_CINEMATIC);
  	for(i=0; i < 256; i++) {
	    codebook2[i] = (VQDATA *) Mem_ClearedAlloc(16*sizeof(VQDATA), TAG_CINEMATIC);
	}
	codebook4 = (VQDATA **) Mem_ClearedAlloc(256*sizeof(VQDATA *), TAG_CINEMATIC);
  	for(i=0; i < 256; i++) {
	    codebook4[i] = (VQDATA *) Mem_ClearedAlloc(64*sizeof(VQDATA), TAG_CINEMATIC);
	}
	previousImage[0] = 0;
	previousImage[1] = 0;
	image = 0;
...
void codec::Segment( int *alist, float *flist, int numElements, float rmse)
{
	...
	lineout = (byte *)Mem_ClearedAlloc( 4*1024, TAG_CINEMATIC);
	...
	if ( doopen ) {
		...
		cbook = (byte *)Mem_ClearedAlloc( 3 * image->pixelsWide() * image->pixelsHigh(), TAG_CINEMATIC);
		float *snrBook = (float *)Mem_ClearedAlloc( image->pixelsWide() * image->pixelsHigh(), TAG_CINEMATIC);
		dst = cbook;
		int numEntries = 0;
...
bool diff;
	...
	idataA = (byte *)Mem_Alloc( 16*16*4, TAG_CINEMATIC);
...
byte *lutimage;
	...
	if (qStatus) Mem_Free(qStatus);
	qStatus =  (quadcel *)Mem_ClearedAlloc(numQuadCels*sizeof (quadcel), TAG_CINEMATIC);
	InitQStatus();
//
	if (previousImage[0]) {
		pixelsWide = previousImage[0]->pixelsWide();
		pixelsHigh = previousImage[0]->pixelsHigh();
		temp = ((whichFrame+1)&1);
		if (!luti) luti = (byte *)Mem_Alloc(pixelsWide*pixelsHigh, TAG_CINEMATIC);
...
byte *idataA, *idataB;
	...
	if (lownum != -1) {
		...
		newsnr = 0;
		idataA = (byte *)Mem_Alloc(8*8*4, TAG_CINEMATIC);
		idataB = (byte *)Mem_Alloc(8*8*4, TAG_CINEMATIC);
...
byte *idataA, *idataB;
	...
	idataA = (byte *)Mem_Alloc( 16*16*4, TAG_CINEMATIC);
	idataB = (byte *)Mem_Alloc( 16*16*4, TAG_CINEMATIC);

	if (!previousImage[0]) common->Printf("sparseEncode: sparsely encoding a %d,%d image\n", pixelsWide, pixelsHigh);
	InitImages();

	flist = (float *)Mem_ClearedAlloc( (numQuadCels+1) *sizeof(float), TAG_CINEMATIC);
	ilist = (int *)Mem_ClearedAlloc( (numQuadCels+1) *sizeof(int  ), TAG_CINEMATIC);
...
void codec::VQ( const int numEntries, const int dimension, const unsigned char *vectors, float *import, VQDATA **codebook, const bool optimize ) {
	int		startMsec = sys->Milliseconds();
